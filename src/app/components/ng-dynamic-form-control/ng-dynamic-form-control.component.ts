...
export class NgDynamicFormControlComponent implements OnInit {
  ...
  form_title: string;
  @Input()
  input_dynamicControls: IDynamicControl;
  @Output()
  ...
  submit() {
    ...
    if (element.length > 0) {
      const extracted: { errors: any[]; data: any[] } = this.extract(element);
      this.onSubmit.emit(extracted);
      if (extracted.errors.length > 0) {
        extracted.errors.forEach((e) => {
          const el = document.getElementById(e[0]["id"]);
  ...
  private extract(elements: HTMLCollectionOf<Element>) {
    ...
    if (this.generatedControls.length === elements.length) {
      for (let i = 0; i < elements.length; i++) {
        ...
        const controlParentDIV = this.generatedControls[i] +
          this._ID_FORM_GROUP;
        if (element.id === controlParentDIV) {
          ...
          if (
            ...
            if (analyzed.length > 0) {
              extracted.errors.push(analyzed);
            } else {
              extracted.data.push({
